...
{    // Snell's law
  ...
  return k < 0 ? vec3 { 1, 0, 0 }
               : I * eta + N * ( eta * cosi - std::sqrt( k ) );    // k<0 = total reflection, no ray to refract. I
                                                                   // refract it anyways, this has no physical meaning
}

bool scene_intersect( const vec3 & orig, const vec3 & dir, const std::vector<Sphere> & spheres, vec3 & hit, vec3 & N,
                      Material & material )
{
  ...
  {                                        // avoid division by zero
    ...
    vec3  pt = orig + dir * d;
    if( d > 1e-3 && std::fabs( pt.x ) < 10 && pt.z < -10 && pt.z > -30 && d < spheres_dist )
    {
      checkerboard_dist = d;
      hit               = pt;
      N                 = vec3 { 0, 1, 0 };
      material.diffuse_color =
          ( ( int( .5 * hit.x + 1000 ) + int( .5 * hit.z ) ) & 1 ) != 0 ? vec3 { .3, .3, .3 } : vec3 { .3, .2, .1 };
    }
  }
  ...
  for( const Light light : lights )
  {
    ...
    if( scene_intersect( point, light_dir, spheres, shadow_pt, trashnrm, trashmat )
        && ( shadow_pt - point ).norm()
               < ( light.position - point ).norm() )    // checking if the point lies in the shadow of the light
      continue;

    diffuse_light_intensity += light.intensity * std::max( 0.F, light_dir * N );
    specular_light_intensity +=
        std::pow( std::max( 0.F, -reflect( -light_dir, N ) * dir ), material.specular_exponent ) * light.intensity;
  }
  ...
  {    // actual rendering loop
    for( size_t i = 0; i < width; i++ )
    {
      ...
      framebuffer[i + j * width] =
          cast_ray( vec3 { 0, 0, 0 }, vec3 { dir_x, dir_y, dir_z }.normalize(), spheres, lights );
    }
  }
